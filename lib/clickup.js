...
// Elvégzett (lezárt) feladatok egy időszakra – date_done szerint
export async function getCompletedTasks({ fromMs, toMs }){
  const out=[];
  let priv = new Set();
  try{ priv = await getPrivateSpaceIds(); }catch(e){ /* nem szűrünk vakon */ }
  for(let page=0; page<20; page++){
    const url = new URL(`${BASE}/team/${TEAM}/task`);
    url.searchParams.set("subtasks","true");
    url.searchParams.set("include_closed","true");
    url.searchParams.set("order_by","date_done");
    url.searchParams.set("reverse","true");
    if(fromMs) url.searchParams.set("date_done_gt", String(Math.floor(fromMs)-1));
    if(toMs)   url.searchParams.set("date_done_lt", String(Math.floor(toMs)+1));
    url.searchParams.set("page", String(page));
    const r = await fetch(url, { headers: headers(), cache:"no-store" });
    ...
    const tasks = d.tasks||[];
    for(const t of tasks){
      const done = t.date_done ? Number(t.date_done) : (t.date_closed? Number(t.date_closed) : null);
      if(!done) continue;
      if(fromMs && done < fromMs) continue;
      if(toMs && done > toMs) continue;
      const spaceId = t.space ? String(t.space.id) : null;
      if(spaceId && priv.has(spaceId)) continue;   // lakatos tér – kihagyjuk
      out.push({
        id:t.id, name:t.name, url:t.url,
