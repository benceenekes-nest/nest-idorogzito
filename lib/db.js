...
export async function saveDayMeta({ email, date, location, partial=false, missingMinutes=0, reason=null }){
  await ensureSchema();
  if(!location) return null;
  const p = !!partial;
  const mm = p ? Math.max(0, Math.min(480, Number(missingMinutes)||0)) : 0;
  const rs = p ? (reason||null) : null;
  await sql`INSERT INTO work_days (user_email, work_date, location, partial, missing_minutes, reason)
    VALUES (${email}, ${date}, ${location}, ${p}, ${mm}, ${rs})
    ON CONFLICT (user_email, work_date)
    DO UPDATE SET location = EXCLUDED.location, partial = EXCLUDED.partial,
      missing_minutes = EXCLUDED.missing_minutes, reason = EXCLUDED.reason, updated_at = now()`;
  return location;
}
