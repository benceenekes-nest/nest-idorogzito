...
export async function ensureSchema(){
  ...
  await sql`ALTER TABLE work_days ADD COLUMN IF NOT EXISTS late_reason TEXT`;
  await sql`ALTER TABLE work_days ADD COLUMN IF NOT EXISTS early_reason TEXT`;
...
// Napi lap fejadatai: hely + nem teljes munkanap
export async function saveDayMeta({ email, date, location, partial=false, missingMinutes=0, lateReason=null, earlyReason=null }){
  await ensureSchema();
  if(!location) return null;
  const p = !!partial;
  const mm = p ? Math.max(0, Math.min(480, Number(missingMinutes)||0)) : 0;
  const lr = p ? (lateReason||null) : null;
  const er = p ? (earlyReason||null) : null;
  await sql`INSERT INTO work_days (user_email, work_date, location, partial, missing_minutes, late_reason, early_reason)
    VALUES (${email}, ${date}, ${location}, ${p}, ${mm}, ${lr}, ${er})
    ON CONFLICT (user_email, work_date)
    DO UPDATE SET location = EXCLUDED.location, partial = EXCLUDED.partial,
      missing_minutes = EXCLUDED.missing_minutes, late_reason = EXCLUDED.late_reason,
      early_reason = EXCLUDED.early_reason, updated_at = now()`;
  return location;
}

export async function getDayMeta({ email, date }){
  await ensureSchema();
  const { rows } = await sql`SELECT location, partial, missing_minutes, late_reason, early_reason
