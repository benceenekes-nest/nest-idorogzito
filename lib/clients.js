...
// Régi/al-lista nevek összevonása a fő ügyfélhez (visszamenőleg a tárolt neveken is).
export const CLIENT_ALIASES = {
  "Modern Gyárak Éjszakája 2026": "MGFÜ",
};
export function normClient(name){
  if(!name) return name;
  return CLIENT_ALIASES[name] || name;
}

export function clientOf(task){
  const lid = task && task.list && task.list.id;
  const raw = (lid && CLIENTS[lid]) || (task && task.list && task.list.name) || "—";
  return normClient(raw);
}
